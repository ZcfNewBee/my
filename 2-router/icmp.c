...
void icmp_send_packet(const char *in_pkt, int len, u8 type, u8 code)
{
    ...
    u32 out_daddr = ntohl(in_ip->saddr);
    ...
    if (type != ICMP_ECHOREPLY)
        tot_size = ETHER_HDR_SIZE + IP_BASE_HDR_SIZE + ICMP_HDR_SIZE + IP_HDR_SIZE(in_ip) + 8;
    else
        tot_size = len - IP_HDR_SIZE(in_ip) + IP_BASE_HDR_SIZE;
