...
void arpcache_insert(u32 ip4, u8 mac[ETH_ALEN]) {
    ...
    if (avil_pos == -1) {
        ...
        avil_pos = rand() % MAX_ARP_SIZE;
    }


    arpcache.entries[avil_pos].ip4 = ip4;
    memcpy(arpcache.entries[avil_pos].mac, mac, ETH_ALEN);
    arpcache.entries[avil_pos].added = time(NULL);
    arpcache.entries[avil_pos].valid = 1;


    struct arp_req *req_entry = NULL, *req_q;
    list_for_each_entry_safe(req_entry, req_q, &(arpcache.req_list), list){
        if(req_entry->ip4 == ip4){
            struct cached_pkt *pkt_entry = NULL, *cached_q;
            list_for_each_entry_safe(pkt_entry, cached_q, &(req_entry->cached_packets), list){
